...
def limpiar_texto(texto):
    """Limpia caracteres no compatibles (como emojis) para que FPDF no arroje error."""
    if not texto: return ""
...
def generar_pdf_profesional(datos_rutina, datos_nutricion, config, cliente, logo_file, estilo):
    ...
    def dibujar_pagina(titulo_pagina, datos_dict, tipo_modulo):
        ...
        for dia in dias:
            ...
            # Altura ajustada a 29 para permitir que los textos largos bajen de línea
            altura_caja = 29
            if estilo == "Clean Minimal":
                pdf.set_draw_color(0, 0, 0)
            ...
            # Imprimir Contenido (Inteligencia de 2 Columnas y Multi-línea)
            ...
            for idx, item in enumerate(items):
                ...
                x_pos = 63 + (columna * 65) 
                y_pos = y_offset + 2 + (fila * 9) # Más separación entre filas
                
                # Limpiamos emojis para evitar errores
                ...
                if tipo_modulo == "entreno":
                    ...
                    pdf.set_font("Arial", 'B', 8)
                    pdf.cell(60, 4, nom_limpio[:45]) # Límite muy alto solo por seguridad visual
                    ...
                    # Detalle (USANDO MULTI_CELL PARA QUE NO SE CORTE EL TEXTO)
                    pdf.set_xy(x_pos, y_pos + 4)
                    pdf.set_font("Arial", '', 7.5)
                    det_limpio = limpiar_texto(item['detalle'])
                    # multi_cell permite que el texto haga "wrap" a la siguiente línea
                    pdf.multi_cell(62, 3.5, det_limpio) 

            y_offset += 31 # Salto calibrado para que entren las cajas de 29 y no tape el footer
...
tab1, tab2 = st.tabs(["🔥 Módulo de Entrenamiento", "🍎 Módulo de Nutrición"])
...
if st.button("🚀 GENERAR PDF PROFESIONAL", use_container_width=True):
    ...
    st.success("¡Plan generado con éxito! Textos completos y app a prueba de errores.")
    st.download_button(
